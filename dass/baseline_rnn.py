...
# [X_val, Y_val] = pickle.load(open('DUMMY.pkl'))
# X_tr = X_val
# X_test = X_val
# Y_tr = Y_val
# Y_test = Y_val
...
for iteration in xrange(EPOCH):
	...
	for i in xrange(N_tr):
		...
		eh = model.fit([x_q,x_a], y, batch_size = 1, nb_epoch=1, verbose = 0)
	...
	for i in xrange(N_val):
		...
		e_val_loss += [model.evaluate([x_q,x_a], y, batch_size = 1, verbose = 0)]
...
for i in xrange(len(X_test)):
	...
	test = model.predict([x_q,x_a], batch_size = 1, verbose = 0)
